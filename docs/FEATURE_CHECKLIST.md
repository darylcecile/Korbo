...
- [x] P0 Landscape‑first layout; portrait collapses to center + drawers — compact = chat only with sessions (left) + context (right) overlay drawers; medium = sessions inline + context drawer. **iPhone supported** (`TARGETED_DEVICE_FAMILY: "1,2"`): a phone is always the `compact` width, so it reuses the Stage‑Manager compact layout (chat full‑width + hamburger sessions/context drawers); verified on an iPhone 17 Pro sim — launches, renders the full conversation, and the sessions drawer opens with project picker + session list. Fix: the inline context pane built a `ClosedRange` `300...min(760, width*0.6)` which **inverted on narrow widths** (the first render uses the default `.wide` mode before `onAppear` corrects it, so it ran with an iPhone width and crashed with "Range requires lowerBound <= upperBound"); now guarded via `AppModel.contextPaneInlineWidth(available:)` which wraps `max(lower, …)` around the width‑derived upper bound
...
- [x] P1 Context switchers (instance + project) — two mutually-exclusive sidebar pills, picked by connection type via `cloud.connectedInstance`. **On a Korbo Cloud instance** the pill is a **cloud-instance switcher**: a searchable sheet (`InstancePickerView`) listing instances **labelled by repo** (`CloudInstance.displayName`, falling back to a short id) with a checkmark on the connected one; tapping calls `connectToInstance` (guarded against busy/terminal states), and "Manage instances…" opens the cloud sheet. **On a local/LAN server with >1 project** the pill is a **project switcher** (`ProjectPickerView`) that re-scopes sessions/files/git via `?directory=<dir>`. Both are searchable in the command palette ("Switch to instance" / "Switch project"). **Lock-out fix:** the old project switcher persisted the chosen directory *before* connecting, so an unreachable directory stayed saved and re-broke on every relaunch. Now connection is funnelled through one `establish(server:directory:)` primitive: `connect()` falls back to the server default if a saved directory can't be reached (clearing it only after the default connects), `switchProject()` persists **only on success** and otherwise rolls back to the previous known-good project, and overlapping switches are serialized by an `isReconnecting` guard — so a bad pick can never strand the user. Verified end-to-end against a live multi-project `opencode serve` (switch in/out of the empty `/` global project stays connected and recovers) and on a cloud instance.
...
- [x] P2 Handoff / Continuity — advertises an `NSUserActivity` (`ai.korbo.Korbo.session`, declared in a hand-authored `Info.plist` `NSUserActivityTypes` that merges with the generated keys) describing the selected session's server + id; on continue it switches to the matching server (by id, then normalized URL), connects, and opens the session. Build + Info.plist merge verified on device; full device-to-device Handoff not verifiable on a single simulator (requires two devices on one iCloud account)
