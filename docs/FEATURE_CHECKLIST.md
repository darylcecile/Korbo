...
- [x] P1 Project switcher (multi-project) — folder pill at the top of the sidebar opens a **searchable sheet** (`ProjectPickerView`) listing every project from `GET /project`, filterable by name or path (servers can host many projects, so a dropdown doesn't scale); selecting one re-scopes sessions, files and vcs to that project by injecting `?directory=<dir>` on every request (incl. SSE). The scoped dir is the project's **sandbox** when present (opencode stores sessions against the exact sandbox worktree, not the top-level worktree — querying the worktree alone returns 0 sessions for sandboxed projects), else the worktree; `.git/modules` projects are labelled by their sandbox leaf to avoid five ambiguous "modules" entries. Choice persisted per-server (`korbo.selectedProject.v2`). Verified on device: search filters live, selection switches + dismisses, Korbo→core→Korbo round-trips sessions + git correctly
